...
class ResNet(nn.Module):
    ...
    def __init__(self, block, num_blocks, num_classes=10, feat_scale=1, wm=1, temperature=1.0):
        ...
        #NOTE: penultimate layer must be 512 dimensional, else add a linear layer
        if feat_scale*widths[3]*block.expansion != 512:
            self.added_linear = nn.Linear(feat_scale*widths[3]*block.expansion, 512)
        else:
            self.added_linear = nn.Identity()

        self.linear = nn.Linear(penultimate_dim, num_classes)
...
#def test():
#    net = resnet18()
#    #_, y = net(torch.randn(1,3,32,32), with_latent=true)
#    #print(y.size())
#
