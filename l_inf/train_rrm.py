...
def get_args():
    parser = argparse.ArgumentParser(description='RRM training script for CIFAR-10 under the l_infty threat model.')
    ...
    parser.add_argument('--lr-schedule', default='cosine', type=str, choices=['cyclic', 'multistep', 'cosine'])
    ...
    parser.add_argument("--t-loadpath", type=str, default="/path/to/teacher/checkpoint.pt")
    parser.add_argument("--xent-weight", type=float, default=0.005)
...
def main():
    ...
    ckpt = load_checkpoint(args.t_loadpath, 'eval', device)
    t_model.net.load_state_dict(ckpt['model_sd'])
    print(f"Successfully loaded teacher from epoch: {ckpt['load_epoch']} !!!")
    del ckpt
    t_model.eval()
